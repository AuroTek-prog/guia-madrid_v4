async function renderPage() {
    // Obtener el apartamento actual y su ciudad
    let apartmentId = (window.appState && window.appState.apartmentId) || (new URLSearchParams(window.location.search).get('apartment')) || 'sol-101';
    let apartmentsData;
    try {
        const res = await fetch(`${window.ROOT_PATH}data/apartments.json`);
        apartmentsData = await res.json();
    } catch (e) {
        apartmentsData = {};
    }
    const apt = apartmentsData[apartmentId] || {};
    const cityId = apt.cityId || 'madrid';
    // Cargar el archivo de datos de la ciudad correspondiente
    let cityData;
    try {
        const res = await fetch(`${window.ROOT_PATH}data/${cityId}.json`);
        cityData = await res.json();
    } catch (e) {
        cityData = null;
    }
    if (!cityData) {
        document.getElementById('page-title').textContent = 'No se pudo cargar la guía turística.';
        return;
    }
    ...
    document.getElementById('hero-headline').textContent = cityData.hero && cityData.hero.headline ? cityData.hero.headline : (cityId.charAt(0).toUpperCase() + cityId.slice(1)) + ' Te Espera';
    ...
    // Imagen principal
    if (cityData.hero && cityData.hero.image) {
    ...
    (cityData.experiences || []).forEach((exp, index) => {
        ...
        if (detailsButton) {
            detailsButton.addEventListener('click', function() {
                ...
                const searchQuery = encodeURIComponent(`${placeTitle} ${cityId.charAt(0).toUpperCase() + cityId.slice(1)}`);
    ...
    setupBottomNavigation(apartmentId, window.appState && window.appState.lang ? window.appState.lang : 'es');
}
            // Actualizar textos estáticos
            document.title = t('tourism.title');
            document.getElementById('page-title').textContent = t('tourism.title');
